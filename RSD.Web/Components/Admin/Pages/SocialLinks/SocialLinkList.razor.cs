...
public partial class SocialLinkList(ISocialLinkService Service, IToastService Toasts) : ComponentBase
{
    private List<SocialLink> Items { get; set; } = [];
    ...
    private async Task ReloadAsync()
    {
        ...
        Items = list.OrderBy(s => s.Scope).ThenBy(s => s.DisplayOrder).ThenBy(s => s.Label).ToList();
    }
    ...
    private async Task ReorderAsync(Guid id, int delta)
    {
        ...
        var entries = Items.Select((s, i) => new ReorderEntry(s.Id, i + 1)).ToList();
    ...
    {
        ApplyOutcome(await Service.SoftDeleteAsync(id, CancellationToken.None), "Link deleted.");
        await ReloadAsync();
    }
