...
public partial class MissionStatList(IMissionStatService Service, IToastService Toasts) : ComponentBase
{
    private List<MissionStat> Items { get; set; } = [];
    ...
    private async Task ReloadAsync()
    {
        ...
        Items = list.OrderBy(s => s.DisplayOrder).ThenBy(s => s.Label).ToList();
    }
    ...
    {
        ApplyOutcome(await Service.SoftDeleteAsync(id, CancellationToken.None), "Stat deleted.");
        await ReloadAsync();
    }
