...
public partial class ValueList(IValueService Service, IToastService Toasts) : ComponentBase
{
    private List<Value> Items { get; set; } = [];
    ...
    private async Task ReloadAsync()
    {
        ...
        Items = list.OrderBy(v => v.DisplayOrder).ThenBy(v => v.Title).ToList();
    }
    ...
    private async Task ReorderAsync(Guid id, int delta)
    {
        ...
        var entries = Items.Select((v, i) => new ReorderEntry(v.Id, i + 1)).ToList();
    ...
    {
        ApplyOutcome(await Service.SoftDeleteAsync(id, CancellationToken.None), "Value deleted.");
        await ReloadAsync();
    }
