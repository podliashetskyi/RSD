...
public partial class PartnerList(IPartnerService Service, IToastService Toasts) : ComponentBase
{
    private List<Partner> Items { get; set; } = [];
    ...
    private async Task ReloadAsync()
    {
        ...
        Items = list.OrderBy(p => p.DisplayOrder).ThenBy(p => p.Name).ToList();
    }
    ...
    private async Task ReorderAsync(Guid id, int delta)
    {
        ...
        var entries = Items.Select((p, i) => new ReorderEntry(p.Id, i + 1)).ToList();
    ...
    {
        ApplyOutcome(await Service.SoftDeleteAsync(id, CancellationToken.None), "Partner deleted.");
        await ReloadAsync();
    }
